...
class GomokuWindow(QMainWindow):
        ...
        if e.button() == Qt.LeftButton:
            if not self.AIplaying:
                if self.res != 0:
                    ...
                    return
                self.AIplaying = True
                t1 = threading.Thread(target=self.playing)
                t1.start()
    ...
    def end_flash(self):
        ...
        if self.flash_cnt <= 5:
            ...
            # 1. 显示游戏结束的信息
            if self.res == 1:
                QMessageBox.about(self, '游戏结束', 'AI1(黑)获胜!')
            elif self.res == 2:
                QMessageBox.about(self, '游戏结束', 'AI2(白)获胜!')
            elif self.res == 3:
