...
class GT_Tree:
    ...
    def search_best(self, input_map):
        ...
        sort_list = [i for i in self.root.childs if i.beta != self.init_beta]    # 过滤掉被启发函数过滤的点
        ...
        for factor in self.root.childs:
            ...
            #for fa in factor.childs:
                #if not fa.is_leaves():
                   # print(2)
        self.choose_node = sort_list[0]         # 最佳节点
    ...
    def extend_node(self, input_node, deepth, max_deepth, what_turn):
        ...
        if deepth < max_deepth:
            # 不是最大深度
            length = len(tmp_list)
            tmp_list = tmp_list[:length // 10 + 1]   # 只保留估值前1/3 的节点数，启发式搜索
